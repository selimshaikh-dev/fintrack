...
{
    public class UpdateMenusUrlCommandHandler : IRequestHandler<UpdateMenusUrlCommand, ResultModel>
    {
        private readonly IMenusUrlService _menusUrlService;
        private readonly IMapper _mapper;
        public UpdateMenusUrlCommandHandler(IMapper mapper, IMenusUrlService menusUrlService)
        {
            _menusUrlService = menusUrlService ?? throw new ArgumentNullException(nameof(_menusUrlService));
            _mapper = mapper ?? throw new ArgumentNullException(nameof(_mapper));
        }
        public async Task<ResultModel> Handle(UpdateMenusUrlCommand request, CancellationToken cancellationToken)
        {
            var role = new MenusUrlVM { Id = request.Id, Name = request.Name };
            var result = await _menusUrlService.UpdateMenusUrl(role);
            return result;
        }
    }
}
